import logging
import numpy as np
from .tools.utils_thesis import igw_disp_rel

def f_range(Nt, dt=1):
    Lt = Nt*dt
    logging.info("f -- Lt: {}, Nt: {}".format(Lt, Nt))
    delta_om = 1/Lt
    N_om = (Nt-1)/2
    return delta_om*np.arange(0,N_om+1)
...
def create_filter_k(k_, Lt_km=50, order=0, flip=False): #Orden 0 es el filtro ideal, de 1 de adelante es el Butterworth
    K_max = 1/Lt_km
    kx_,ky_ = k_
...
def create_filter_igw(k_, Nbv, Lt_km=50, order=0,flip=False): #Orden 0 es el filtro ideal, de 1 de adelante es el Butterworth
    K_max = 1/Lt_km
    kx_,ky_ = k_
...
def create_filter_om(Nt, dt_h=1.0, cuttoff_h=30, order=0, flip=False):
    # 
    Lt = Nt*dt_h
    delta_om = 1/Lt
    N_om = (Nt-1)/2
...
def filter_fft_butterworth_kh(var_xy,Lt_km=50,dx=2,dy=2,order=100):
    ...
    filter_mask = create_filter_k(k_, Lt_km=Lt_km, order=order, flip=True)
...
def filter_fft_butterworth_omega(var_txy,cuttoff_h=30,dt_h=1,order=100,axis=-1):
    ...
    filter_mask = create_filter_om(var_txy.shape[axis], dt_h=dt_h, cuttoff_h=cuttoff_h,  order=order, flip=True)
